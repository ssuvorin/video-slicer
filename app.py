...
class App:
    def __init__(self, root: Tk):
        ...
        Label(root, text="Видео файл:").grid(row=0, column=0, sticky="w", padx=8, pady=6)
        Entry(root, textvariable=self.video_path_var, width=50).grid(row=0, column=1, padx=8, pady=6)
        Button(root, text="Выбрать...", command=self.choose_video).grid(row=0, column=2, padx=8, pady=6)

        Label(root, text="Папка вывода:").grid(row=1, column=0, sticky="w", padx=8, pady=6)
        Entry(root, textvariable=self.output_dir_var, width=50).grid(row=1, column=1, padx=8, pady=6)
        Button(root, text="Папка...", command=self.choose_output_dir).grid(row=1, column=2, padx=8, pady=6)

        Label(root, text="Длина сегмента (сек):").grid(row=2, column=0, sticky="w", padx=8, pady=6)
        Entry(root, textvariable=self.segment_len_var, width=10).grid(row=2, column=1, sticky="w", padx=8, pady=6)

        Checkbutton(root, text="Быстро (без перекодирования)", variable=self.fast_copy_var).grid(row=2, column=2, sticky="w", padx=8, pady=6)

        self.progress = ttk.Progressbar(root, orient="horizontal", length=420, mode="determinate")
        self.progress.grid(row=3, column=0, columnspan=3, padx=8, pady=10)

        self.start_button = Button(root, text="Нарезать", command=self.start_slicing)
        self.start_button.grid(row=4, column=0, columnspan=3, padx=8, pady=6)

        self.status_label = Label(root, textvariable=self.status_var)
        self.status_label.grid(row=5, column=0, columnspan=3, sticky="w", padx=8, pady=6)

        # macOS: make sure the window is a reasonable size
    ...
    def start_slicing(self):
        ...
        config = SliceConfig(segment_seconds=seg, fast_copy=self.fast_copy_var.get())
